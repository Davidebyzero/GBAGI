...
static std::vector<ORIGINAL_WORD> g_originalWords;
static std::vector<char*> g_originalWordStrings;
...
/******************************************************************************/
static void FillGbaHeaderText(U8 *dest, int len, const TCHAR *text, const char *fallback)
{
	int i = 0;
	memset(dest, 0, len);
	if(!text || !*text)
	...
	while(*text && i < len) {
		TCHAR ch = *text++;
		if(_istalnum(ch) || ch == _T(' ') || ch == _T('_') || ch == _T('-')) {
			if(ch >= _T('a') && ch <= _T('z'))
...
/******************************************************************************/
static void ResetOriginalWords()
{
	size_t i;
	for(i=0; i<g_originalWordStrings.size(); ++i)
		free(g_originalWordStrings[i]);
	g_originalWordStrings.clear();
	g_originalWords.clear();
}
/******************************************************************************/
static void RememberOriginalWord(int group, const char *string)
{
	ORIGINAL_WORD ow;
	char *copy;
	if(!string || !string[0])
	...
	ow.group = group;
	ow.string = copy;
	g_originalWordStrings.push_back(copy);
	g_originalWords.push_back(ow);
}
/******************************************************************************/
static int CountPlanExtraAliases()
{
	int count = 0;
	VOCAB_PLAN_ENTRY *p;
	if(!gi || !gi->vocabPlan)
		return 0;
	for(p = gi->vocabPlan; p; p = p->next) {
		if(p->addAlias && p->word && p->word[0])
			count++;
	}
	return count;
}
/******************************************************************************/
static int GetCurrentWordsetCount()
{
	int count = 0;
	WORDSET *w = wordset;
	while(w && w->group) {
...
/******************************************************************************/
static WORDSET *FindCurrentWord(int group, const char *string)
{
	WORDSET *w = wordset;
	while(w && w->group) {
		if((w->group & 0x1FFF) == group && strcmp(w->string, string) == 0)
			return w;
		w++;
	}
	return NULL;
...
/******************************************************************************/
static const ORIGINAL_WORD *FindOriginalWordEntry(int group, const char *string)
{
	size_t i;
	for(i=0; i<g_originalWords.size(); ++i) {
		if((g_originalWords[i].group & 0x1FFF) == group && strcmp(g_originalWords[i].string, string) == 0)
			return &g_originalWords[i];
	}
	return NULL;
}
/******************************************************************************/
static void RemoveCurrentWord(int group, const char *string)
{
	WORDSET *w = wordset;
	while(w && w->group) {
		if((w->group & 0x1FFF) == group && strcmp(w->string, string) == 0) {
...
/******************************************************************************/
static void RemoveCurrentGroup(int group)
{
	WORDSET *w = wordset;
	while(w && w->group) {
		if((w->group & 0x1FFF) == group) {
			WORDSET *next = w + 1;
			while(next->group) {
				*w = *next;
				w++;
...
/******************************************************************************/
static BOOL SetCurrentWordColumn(int group, const char *string, int columnOverride)
{
	WORDSET *w = wordset;
	BOOL found = FALSE;
	while(w && w->group) {
...
/******************************************************************************/
static void AddWordWithColumn(int group, const char *string, int columnOverride, int originalGroup)
{
	int finalGroup = group;
	if(columnOverride == 0 || columnOverride == 1)
		finalGroup = group | ((columnOverride == 0) ? 0x8000 : 0);
	else if(originalGroup)
		finalGroup = originalGroup;
	AddWord(finalGroup, string);
}
/******************************************************************************/
static void ApplyGenericVocabPlan()
{
	VOCAB_PLAN_ENTRY *p;
	if(!gi || !gi->vocabPlan)
		return;
	...
	for(p = gi->vocabPlan; p; p = p->next) {
		if(p->replaceGroup && p->group > 0)
			RemoveCurrentGroup(p->group);
	}

	for(p = gi->vocabPlan; p; p = p->next) {
		const ORIGINAL_WORD *orig = NULL;
		int baseGroup;
		if(!p->word || !p->word[0])
			continue;
		baseGroup = p->group & 0x1FFF;
		if(baseGroup <= 0)
			continue;

		if(p->forceRemove)
			RemoveCurrentWord(baseGroup, p->word);

		orig = FindOriginalWordEntry(baseGroup, p->word);

		if(p->columnOverride == 0 || p->columnOverride == 1) {
			if(!p->forceRemove) {
				if(!SetCurrentWordColumn(baseGroup, p->word, p->columnOverride)) {
				int originalGroup = orig ? orig->group : baseGroup;
				AddWordWithColumn(baseGroup, p->word, p->columnOverride, originalGroup);
				}
			}
			continue;
		}
		...
		if(p->addAlias) {
			if(!FindCurrentWord(baseGroup, p->word))
				AddWordWithColumn(baseGroup, p->word, -1, 0);
			continue;
		}

		if(p->forceKeep && !FindCurrentWord(baseGroup, p->word)) {
			int originalGroup = orig ? orig->group : baseGroup;
			AddWordWithColumn(baseGroup, p->word, -1, originalGroup);
		}
	}
}
/******************************************************************************/
static BOOL IsWordHiddenByPlan(int group, const char *string)
{
	VOCAB_PLAN_ENTRY *p;
	if(!gi || !gi->vocabPlan || !string)
		return FALSE;
	for(p = gi->vocabPlan; p; p = p->next) {
		if(!p->word || !p->word[0])
			continue;
		if((p->group & 0x1FFF) == (group & 0x1FFF) && strcmp(p->word, string) == 0)
			return p->hidden ? TRUE : FALSE;
	}
	return FALSE;
}
/******************************************************************************/
static int GetPlanPickerVisibility(int group, const char *string)
{
	VOCAB_PLAN_ENTRY *p;
	if(!gi || !gi->vocabPlan || !string)
		return -1;
...
/******************************************************************************/
static int GetPlanColumnOverride(int group, const char *string)
{
	VOCAB_PLAN_ENTRY *p;
	if(!gi || !gi->vocabPlan || !string)
		return -1;
...
BOOL ProcessWords()
{
    ...
    for(i=0;i<26;i++) {
        ...
        do {
            ...
            g=beGetW(msg);
            if(g!=1&&g!=9999) {
            	RememberOriginalWord(g, wordbuf);
            	l=strlen(wordbuf)+1;
               	wPtr[0] = l+3;
            	wPtr[1] = g&0xFF;
            	wPtr[2] = g>>8;
    ...
    DoRemainingWords();
    ApplyGenericVocabPlan();
    //if(pwords!=wordset)
    	AlphaSortWords();
    if(IsSpaceQuest2()) {
    	FILE *dbg=fopen("H:\\CODEX\\GBAGI\\_sq2_g43_debug.txt","w");
    	if(dbg) {
    		WORDSET *w = wordset;
    		while(w->group) {
    			if((w->group & 0x1FFF) == 43)
    				fprintf(dbg,"%05d:%s\n", w->group & 0x1FFF, w->string);
    			w++;
    		}
    		fclose(dbg);
    	}
...
BOOL OutputGame()
{
    ...
    w = wordset;
    while(w->group) {
        int columnOverride = GetPlanColumnOverride(w->group & 0x1FFF, w->string);
        if(columnOverride == 0)
            w->group |= 0x8000;
        else if(columnOverride == 1)
            w->group &= 0x7FFF;
    ...
    *wPtr++ = 0;
    ...
    if(wBuf) {
        wordFlagsBuf = (U8*)calloc(wSize, 1);
        if(wordFlagsBuf) {
            w = wordset;
            while(w && w->group) {
                long offset = (long)((char*)w->string - 3 - wBuf);
                if(offset >= 0 && offset < wSize) {
                    int pickerVisibility = GetPlanPickerVisibility(w->group & 0x1FFF, w->string);
                    if(IsWordHiddenByPlan(w->group & 0x1FFF, w->string))
                        wordFlagsBuf[offset] |= 0x01;
                    if(pickerVisibility == 0)
                        wordFlagsBuf[offset] |= 0x02;
                    else if(pickerVisibility == 1)
                        wordFlagsBuf[offset] |= 0x04;
                }
                w++;
            }
        }
    ...
    ptrs[11] = offs;
    if(wordFlagsBuf) {
        fwrite(wordFlagsBuf, wSize, 1, fout);
        offs += wSize;
    } else {
        fputl(0, fout);
        offs += 4;
